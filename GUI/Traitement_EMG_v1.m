function varargout = Traitement_EMG_v1(varargin)
% TRAITEMENT_EMG_V1 MATLAB code for Traitement_EMG_v1.fig
%      TRAITEMENT_EMG_V1, by itself, creates a new TRAITEMENT_EMG_V1 or raises the existing
%      singleton*.
%
%      H = TRAITEMENT_EMG_V1 returns the handle to a new TRAITEMENT_EMG_V1 or the handle to
%      the existing singleton*.
%
%      TRAITEMENT_EMG_V1('CALLBACK',hObject,eventdata,handles,...) calls the local
%      function named CALLBACK in TRAITEMENT_EMG_V1.M with the given input arguments.
%
%      TRAITEMENT_EMG_V1('Property','Value',...) creates a new TRAITEMENT_EMG_V1 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Traitement_EMG_v1_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Traitement_EMG_v1_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Traitement_EMG_v1

% Last Modified by GUIDE v2.5 03-Sep-2014 16:29:32

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @Traitement_EMG_v1_OpeningFcn, ...
    'gui_OutputFcn',  @Traitement_EMG_v1_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before Traitement_EMG_v1 is made visible.
function Traitement_EMG_v1_OpeningFcn(hObject, ~, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user Data (see GUIDATA)
% varargin   command line arguments to Traitement_EMG_v1 (see VARARGIN)
global haxes1 haxes2

% Choose default command line output for Traitement_EMG_v1
...
% UIWAIT makes Traitement_EMG_v1 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = Traitement_EMG_v1_OutputFcn(~, ~, handles)
...
Tag_sujet = upper(files{1}(ind_nom_sujet(2)+1:ind_nom_sujet(3)-1));
    ...
    for j_session = 1 : length(liste_session)
        ...
        if exist(path,'dir')==7
            for k_condition = 1 : length(liste_condition)
                ...
                calculer_Callback;
                bouffees;
                sauvegarder('D:\01_GBMOV\01_Traitement\04_EMG\Traitement');
                %     export_figure;
    ...
    for j_emg = 1 : length(list_EMG)
        if get(findobj('Tag','TKEO'), 'Value')
            ...
            elseif get(findobj('Tag','fenetre_bruit'), 'Value')
                [~,b]=matchcells({nom_acq},get(findobj('Tag','listbox1'),'String'));
